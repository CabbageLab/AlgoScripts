...
func main() {
	file, err := xlsx.OpenFile("./Golang_Basis/openai_chat/bible_translation/daily verse_to_de_fr.xlsx")
	if err != nil {
		log.Fatal(err)
	}
	...
	for rowIndex, row := range sheet.Rows {
		...
		originalText := row.Cells[11].String()
		...
		// 调用OpenAI API进行重写
		rewrittenText := rewriteWithOpenAI(originalText, "fr")
	...
	// 保存修改后的Excel文件
	err = file.Save("./Golang_Basis/openai_chat/bible_translation/daily_output_inspi_fr.xlsx")
	if err != nil {
		log.Fatal(err)
	}
}
