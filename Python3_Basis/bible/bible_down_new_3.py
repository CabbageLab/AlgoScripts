...
# URL of the Bible Gateway page for Genesis 1 (ESV)
url = "https://www.biblegateway.com/passage/?search=Song%20of%20Songs%201&version=NABRE"
    ...
    if response.status_code == 200:
        ...
        if passage_div:
            ...
            for i in range(0, len(verses)):
                ...
                # for MW in soup.find_all('b', class_='speaker'):
                #     if MW.text=='M':
                #         MW.string="@"
                #     else:
                #         MW.string="$"
                ...
                span_text = verse.get_text().strip().replace("\n", "")
                print("dfasdfasdf"+span_text)
                if i == 0:
                    title = span_text
                    continue
                # 如果class_name已经存在于字典中，拼接内容
                if class_name in content_dict:
                    ...
                    if verse.find('sup') or verse.find('span'):
                        ...
                        content_dict[class_name + "_1"] = "#" + span_text
            # Create a directory for the book if it doesn't exist
            book_dir = f"./{book_name}"
...
if __name__ == '__main__':
    getUrlContent("Genesis_Test", "Genesis 1", url)
