...
def scrape_bible(section_name, book_name, chapter_name, index):
    base_url = 'https://www.bible.com/bible/2692/' + section_name + '.' + str(
        chapter_name) + '.NASB2020'
    print(base_url)
    ...
    for div in div_list:
        ...
        # outer_span = div.find('span', class_='ChapterContent_it__p_cmC')
        # # 在外层 span 中查找内层 span，确保匹配 class 且有文本内容
        # if outer_span:
        #     inner_span = outer_span.find('span', class_='ChapterContent_heading__xBDcs')
        #     if inner_span and inner_span.get_text(strip=True):  # 检查内层是否有文本内容
        #         print(inner_span.get_text(strip=True))  # 输出文本内容
        #         title = inner_span.get_text()
        #     else:
        #         print("未找到有文本内容的内层 span")
        # else:
        #     print("未找到外层 span")

        # 查找第一个 class 为 ChapterContent_it__p_cmC 的 span 标签

        for sup_tag in div.find_all('span', class_='ChapterContent_note__YlDW0'):
            sup_tag.decompose()  # 完全删除 <sup> 标签及其内部内容

        inner_span = div.find_all('span', class_='ChapterContent_heading__xBDcs')
        # 在该 span 标签内查找第二个 class 为 ChapterContent_heading__xBDcs 的 span 标签
        for ii in inner_span:
            title=None
            if inner_span:
                title = ii.get_text()
            print(ii.get_text())  # 输出：The Creation
            if title.strip():
                print(title)
...
if __name__ == '__main__':
    ...
    with open('./NASB.json', 'r', encoding='utf-8') as f:
        data = json.load(f)
        index = 1
